/*
 * PROTOTYPE — reads/writes the `?variant=` search param for the movies-tv
 * poster prototype. Deliberately not wired into the TanStack router search
 * schema: throwaway plumbing, zero footprint when the param is absent.
 */
import { useCallback, useEffect, useState } from "react";

export type PrototypeVariant = "a" | "b" | "off";
...
function read(): PrototypeVariant | null {
	...
	if (raw === "a" || raw === "b" || raw === "off") return raw;
	return null;
}
