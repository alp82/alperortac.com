/*
 * PROTOTYPE — floating variant switcher for the music snippet-player
 * prototype (wayfinder #27). Renders only when the `?variant=` param is
 * present, so normal browsing and tests never see it.
 */
import { useEffect } from "react";
import type { PrototypeVariant } from "./usePrototypeVariant";

const ORDER: PrototypeVariant[] = ["a", "b", "c", "d", "off"];

const LABELS: Record<PrototypeVariant, string> = {
	a: "A — iTunes preview player (audio)",
	b: "B — YouTube watch-this-moment (video)",
	c: "C — hybrid (audio + expandable moment)",
	d: "D — YouTube inside the grid (2x2 swap)",
	off: "OFF — current page",
};
